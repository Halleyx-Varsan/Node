<template>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="(subject, index) in responseData" :key="subject._id">
              ...
              <td>{{ subject.id }}</td>
              <td>
                <button class="delete-student" @click="deleteStudent(subject.name)">
                  Delete
                </button>
              </td>
            </tr>
  ...
  <script>
  import { agentServer } from "@/config/axiosConfig.js";
  export default {
    data() {
      return {
        showModal: false,
    ...
    methods: {
      ...
      async fetchData() {
        try {
          const response = await agentServer.get("/agents");
      ...
      async postData() {
        try {
          const response = await agentServer.post("/agent", {
      name: this.name,
      email: this.email,
      sec: this.sec,
      std: this.std
      ...
      async deleteStudent(id) {
        try {
          const response = await agentServer.delete(`/agent/${id}`);
          if (response.status === 200) {
            console.log(response.data);
          } else if (response.status === 404) {
            console.log("Invalid Subject ID");
